...
def F_bilinear_interp2d(i_input, coords):
    """
    bilinear interpolation of 2d torch Tensor
    """
    x = torch.clamp(coords[:, :, 0], 0, i_input.size(1) - 2)
    x0 = x.floor()
    x1 = x0 + 1
    y = torch.clamp(coords[:, :, 1], 0, i_input.size(2) - 2)
    y0 = y.floor()
    y1 = y0 + 1

    stride = torch.LongTensor(i_input.stride())
    x0_ix = x0.mul(stride[1]).long()
    x1_ix = x1.mul(stride[1]).long()
    y0_ix = y0.mul(stride[2]).long()
    y1_ix = y1.mul(stride[2]).long()

    input_flat = i_input.view(i_input.size(0), -1).contiguous()
...
def F_batch_bilinear_interp2d(i_input, coords):
    """
    input : torch.Tensor
        size = (N,H,W,C)
    coords : torch.Tensor
        size = (N,H*W*C,2)
    """
    x = torch.clamp(coords[:, :, 0], 0, i_input.size(2) - 2)
    x0 = x.floor()
    x1 = x0 + 1
    y = torch.clamp(coords[:, :, 1], 0, i_input.size(3) - 2)
    y0 = y.floor()
    y1 = y0 + 1
...
def F_trilinear_interp3d(i_input, coords):
    """
    ...
    z = torch.clamp(coords[:, 2], 0, i_input.size(3) - 2)
    z0 = z.floor()
    z1 = z0 + 1

    stride = torch.LongTensor(i_input.stride())[1:]
...
def F_batch_trilinear_interp3d(i_input, coords):
    """
    input : torch.Tensor
        size = (N,H,W,C)
    coords : torch.Tensor
    ...
    y0 = y.floor()
    y1 = y0 + 1
    z = torch.clamp(coords[:, :, 2], 0, i_input.size(4) - 2)
    z0 = z.floor()
    z1 = z0 + 1
    ...
    input_flat = i_input.contiguous().view(i_input.size(0), -1)
